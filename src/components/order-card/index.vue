<template>
  <view class="order-card" @tap="goToOrderDetail">
    ...
    <view class="products">
      <image v-for="(item, index) in order.items.slice(0, 3)" :key="item.id" class="product-img" :src="item.productImage || '/static/default-product.png'" mode="aspectFill" />
      <text class="more" v-if="order.items.length > 3">+{{ order.items.length - 3 }}</text>
    </view>
    <view class="footer">
...
.status--pending_payment { color: $warning; }
